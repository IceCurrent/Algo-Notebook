...
//A student can read only contiguous books and we have to minimise the maximum pages 
...
int solve(int arr[], int i, int j, int k){
    ...
    int ans = INT_MAX;
    for(int r = i; r<j; r++){
        int p = max(sum(arr, i, r), solve(arr, r+1, j, k-1));
        ans = min(ans, p);
    }
...
int dpSolve(int arr[], int k, int n){
    int tb[k+1][n+1];

    for(int i=1; i<=n; i++){
        tb[1][i] = sum(arr, 0, i-1);
    }
    ...
    for(int i=2; i<=k; i++){
        for(int j=2; j<=n; j++){
            int res = INT_MAX;
            for(int r=1; r<j; r++){
                res = min(res, max(sum(arr, r, j-1 ), tb[i-1][r])); 
            }
            tb[i][j] = res;
        }
    } 
    ...
    //time complexity : O(n^3*k) but we can optimise by making a lookup table for sum
    //to make the time complexity O(n^2*k)

    //Aux space: theta(n*k); 

    //for this problem a binary search based solution also exists which has 
    //time complexity as O(nlog(sum)); {but if sum is very high then binary search may
    // be slower than DP solution}
    //sum is basically sum of all elements of the given array;
}
int main(){
    ...
    cout<<solve(arr, 0, 3, k)<<endl;
    cout<<dpSolve(arr, 2, 4)<<endl;
    return 0;
}
