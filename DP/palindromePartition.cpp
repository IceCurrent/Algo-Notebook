...
//this recursive implementation is fine but do try to have solutions where 
//you have a start pointer and an end pointer that really helps
int solve(string s){
    ...
    int ans = INT_MAX;
    fo(i, s.length()-1){
        ans = min(ans, 1 + solve(s.substr(0, i+1)) + solve(s.substr(i+1, s.length()-(i+1))));
    }

    return ans;
}

//This is a more robust recursive implementation
int solve2(string s, int i, int j){
    ...
    int ans = INT_MAX;
    for(int k = i; k<j; k++){
        ans = min(ans, 1+ solve2(s, i, k)+solve2(s, k+1, j));
    }
    return ans;
}


int dpSolve(string s){
    
    int tb[s.length()][s.length()];

    for(int i=0; i<s.length(); i++){
        tb[i][i] = 0;
    }

   //so we consider all gaps and within each gap we consider all possible
   //indexes that are within these gapss
        ...
        for(int i=0; i<s.length()-gap; i++){
            ...
            if(isPal(s.substr(i, j-i+1))){
                ...
                for(int k=i; k<j; k++){
                    ...
                    tb[i][j] = min(tb[i][j], 1+tb[i][k]+tb[k+1][j]);
...
int main(){
    ...
    cout<<solve(s)<<endl;
    cout<<solve2(s, 0, s.length()-1)<<endl;
    cout<<dpSolve(s)<<endl;
    return 0;
}
