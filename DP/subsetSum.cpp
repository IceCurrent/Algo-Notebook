...
//FIND THE NUMBER OF SUBSETS WHOSE SUM IS equal to the given sum

int RecSolve(int arr[], int sum, int n){
    if(sum == 0){
        return 1;
    }
    ...
    return RecSolve(arr+1, sum, n-1)+ RecSolve(arr+1, sum-arr[0], n-1);
}

int dpSolve(int arr[], int sum, int n){
    int tb[n+1][sum+1];
    ...
    for(int i=1; i<=n; i++){
        for(int j=1; j<=sum; j++){
            if(arr[i-1] > j){
                ...
                tb[i][j] = tb[i-1][j-arr[i-1]] + tb[i-1][j]; //In this way the first
                //term tb[i-1][j-arr[i-1]] denotes when we include the current item
                //and tb[i-1][j] denotes the case when we do not include the current item 
            }
        }
    }
...
int main(){
    ...
    cout<<RecSolve(arr, sum, 3)<<endl;
    cout<<dpSolve(arr, sum, 3)<<endl;
     
    //time complexity of dp sol = theta(n*sum);
    //time compledxity of rec sol = theta(2^n);
    return 0;
}
