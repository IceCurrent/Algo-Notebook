...
int solve(vi &price, vi &pages, int n, int x){
    ...
    if(price[n-1] > x){
        ...
        return max(solve(price, pages, n-1, x), solve(price, pages, n-1, x-price[n-1]) + pages[n-1]);
    }

}

int dpSolve(vi &price, vi &pages, int n, int x){
    int tb[x+1];
    ...
    //building the 1D table backwards
    for(int j=0; j<n; j++){
        for(int i=x; i>=0; i--){
                if(i >= price[j]){
                    //deb(i); 
                    tb[i] = max(tb[i], pages[j] + tb[i-price[j]]);
                    //deb(tb[i]);
                }
            }
        }
