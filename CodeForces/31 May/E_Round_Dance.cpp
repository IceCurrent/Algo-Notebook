...
void bfs(vector<vi> adj, int src, vb &vis){
    queue<int> q;
    vis[src] = true;
...
int maxRounds(vector<vi> adj){
    vb vis(adj.size(), false);
    int cnt = 0;

    for(int i=1; i<adj.size(); i++){
        adj[adj[i][0]].push_back(i);
    }
...
int main(){
    ...
    while(tc--){
        ...
        int maxRs = maxRounds(adj);
        int lones = loners(adj);
