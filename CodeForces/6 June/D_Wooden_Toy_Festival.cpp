...
int main(){
    ...
    while(tc--){
        ...
        for(int i=1; i<n; i++){
            diff.pb(meanv[i] - meanv[i-1]);
        }

        int c1, c2, c3;

        int max1 = 0;
        vi::iterator m1 = max_element(all(diff));
        max1 = m1 - diff.begin();

        int max2 = 0;
        fo(i, n-1){
            if(diff[i] > diff[max2] && diff[i] != diff[max1]){
                max2 = i;
            }
        }
        
        c1 = meanDev(v, 0, max2);
        c2 = meanDev(v, max2+1, max1);
        c3 = meanDev(v, max1+1, n-1);
