...
int solve(vector<pii>& v, int cap){
    vector<pair<int, float>> a(v.size());

    fo(i, v.size()){
        a[i].F = v[i].F;
        a[i].S = float(v[i].S)/float(v[i].F);
    }
...
int main(){
    ...
    vector<pii> v = {{50, 600}, {20, 500}, {30, 400}};
    int cap = 70;

    v = {{10, 200}, {5, 50}, {20, 100}};
    cap = 15;

    cout<<solve(v, cap)<<endl;
    return 0;
}
