...
void dfs(vector<vi> &adj, int src, vi &disc, vi &low, vi &parent, vb &vis, int root){
    vis[src] = true;
    disc[src] = k;
    low[src] = k;
    k++;
    ...
    for(int i=0; i<adj[src].size(); i++){
        ...
        if(vis[adj[src][i]] && parent[src] != adj[src][i]){
            //it's a back-edge
        ...
        if(!vis[adj[src][i]]){
            ...
            dfs(adj, adj[src][i], disc, low, parent, vis, root);
            low[src] = min(low[src], low[adj[src][i]]);
            r++;

            if(low[adj[src][i]] >= disc[src] && src != root){
                cout<<src<<"\n";
            }
        }
...
void articulationPts(vector<vi> &adj, int n){
    vi parent(n+1, 0);
    vi disc(n+1, 0);
    ...
    for(int i=1; i<=n; i++){
        root = i;
        if(!vis[i]){
            dfs(adj, i, disc, low, parent, vis, root);
        }
    }

    // cout<<"disc values are: "<<"\n";
    // for(int i=1; i<=n; i++){
    //     cout<<disc[i]<<" ";
    // }
    // cout<<"\n";
