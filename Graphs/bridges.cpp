...
void dfs(vector<vi> &adj, int src, vi &disc, vi &low, vi &parent, vb &vis){
    vis[src] = true;
    disc[src] = k;
    low[src] = k;
    k++;
    ...
    for(int i=0; i<adj[src].size(); i++){
        ...
        if(!vis[adj[src][i]]){
            ...
            dfs(adj, adj[src][i], disc, low, parent, vis);
            low[src] = min(low[src], low[adj[src][i]]);
        }

        if(low[adj[src][i]] > disc[src]){
            cout<<src<<"--"<<adj[src][i]<<"\n";
        }
    }
...
void bridges(vector<vi> &adj, int n){
    vi parent(n+1, 0);
    vi disc(n+1, 0);
